...
ウィンドウは起動時に **64×18 に固定**され、リサイズできなくなります。
画面はその全面を使います。
...
| `Enter` | デバイス行なら一覧を開く。ミュート／ノイズ抑制／AGC 行なら ON/OFF を切り替える |
| `M` | **ミュートの切替**（どの行を選んでいても効く） |
...
```powershell
build\openkrisp.exe --uitest                        # 罫線版（既定 64x18）
...
build\openkrisp.exe --uitest --stress               # 描画の回帰テスト（下記）
```

`--stress` は「デバイス一覧を開くと固まる」不具合の再現テストです。全角文字の
右半分を半角で上書きしてセルの対を壊し、次のフレームの差分描画が走査位置を
戻し続けて抜けられなくなる筋道をなぞります。直っていれば即座に
`screen stress: ok` を出して終わり、壊れていれば CPU を回したまま返りません。
...
- **ウィンドウサイズの固定が効かない端末があります**。conhost ではウィンドウの
  境界とボタンを外して固定できますが、Windows Terminal はタブ型で
  `GetConsoleWindow()` が擬似ウィンドウを返すため、VT でサイズを要求するだけに
  なります（拒否されることがあります）。固定できなかった場合は画面下部にその旨が
  表示され、実サイズのまま描画します。
